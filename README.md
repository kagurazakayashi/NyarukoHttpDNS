...
# Python3客户端
同时也是本地 DNS 服务器，可供局域网中的其它设备一起使用。
...
- `-h` 或 `--no-multithreading`
    - （可选）禁用多线程。这可能会导致因为网络而发生 DNS 请求阻塞，但便于监控 DNS 请求和调试。
...
- 只支持标准 hosts 写法，不支持加入变量或通配符。

## 客户端自定义hosts
- 创建一个文件名为 `hosts.txt` 的文件和 `.python` 文件放置在一起，可为本地DNS服务端自定义hosts。
- 如果没有统一更换的需要，建议使用客户端来进行自定义 hosts，可以获得最佳性能。
- 支持使用以下变量，例如 `<ip> <host>` 可以表达 `192.168.1.233 yashi-pc.local`:
    - `<ip>`: 当前主机内网IP
    - `<null>`: 空白，将直接返回空结果（只允许写在IP栏位）
    - `<host>`: 当前主机在网络中的主机名（只允许写在域名栏位）
    - `<ipurl>`: 反查样式的域名IP（只允许写在域名栏位，例如 `233.1.168.192` 或 `1.0.0.0.0.2.4...` ）
    - `<ipurl0>`: 反查样式的域名IP，域名最后一段为0（只允许写在域名栏位，例如 `0.1.168.192` ）
- 支持通配符，例如 `*.google.com` 可以表达 `plus.google.com`:
    - `*`: 1个或多个字符
    - `?`: 1个字符
